...
shinyServer(function(input, output) {
  ...
  output$indices_metrics <- renderTable({
    data.frame(
      ...
      'S & P 500' = as.character(c(mean(df1[,2]), 
                             sd(df1[,2]),
                             min(df1[,2]),
                             max(df1[,2]))), 
      '3-month T-Bills' = as.character(c(mean(df1[,3]), 
                             sd(df1[,3]),
                             min(df1[,3]),
                             max(df1[,3]))), 
      '10-year Bonds' = as.character(c(mean(df1[,4]), 
                             sd(df1[,4]),
                             min(df1[,4]),
                             max(df1[,4]))), 
      stringsAsFactors=FALSE)  
  ...
  output$metrics <- renderTable({
    data.frame(
      'Portofolio Metrics' = c("Average Return", 
               ...
               "Lowest Yearly Return",
               "Highest Yeaarly Return"),
      Percentage = as.character(c(mean(df2$Portfolio), 
                             sd(df2$Portfolio),
                             min(df2$Portfolio),
                             max(df2$Portfolio))), 
      stringsAsFactors=FALSE)  
